...
impl OctreeLoader<Voxel> for Loader {
    fn load_region_with_lod(&self, region_x: u64, region_y: u64, region_z: u64, lod: u8) -> Option<Octree> {
        if region_z != 0 {
            return None;
        }
        ...
        let file =
            std::fs::File::open(format!("{}/r.{}.{}.mca", "./assets/region", region_x, region_y))
                .unwrap();
...
fn setup_from_oct_file(
    ...
    let loader = Loader {
        block_allocator: render_resources.block_allocator.clone()
    };
