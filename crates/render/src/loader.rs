...
impl OctreeLoader<Voxel> for Loader {
    fn load_region_with_lod(&self, region_x: u64, region_y: u64, region_z: u64, distance_to_center: u8) -> Option<Box<Octree>> {
        ...
        let octree = Octree::new(arena_allocator);
        let mut octree = Box::new(octree);
        let octree_ptr = octree.as_mut() as *mut Octree;
        let octree_ref: &'static mut Octree = unsafe { &mut *octree_ptr };
        std::thread::Builder::new()
            .name(format!("OctreeLoader {} {}", region_x, region_z))
            .spawn(move || {
                let octree = octree_ref; // TODO
                let file =
                    std::fs::File::open(format!("{}/r.{}.{}.mca", "./assets/region", region_x, region_z))
                        .unwrap();
                let region_x = region_x + 4;
                let region_y = region_z + 3;
