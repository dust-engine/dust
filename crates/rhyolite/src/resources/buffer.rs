...
use crate::{
    debug::DebugObject,
    future::{
        CommandBufferRecordContext, GPUCommandFuture, RenderData, RenderRes, SharedDeviceState,
        StageContext,
    },
    macros::commands,
    utils::either::Either,
...
#[pin_project]
pub struct InitializeBufferFuture<
    T: BufferLike + RenderData,
    TRef: DerefMut<Target = RenderRes<SharedDeviceState<T>>>,
> {
    pub dst: TRef,
    data: u32,
}
impl<T: BufferLike + RenderData, TRef: DerefMut<Target = RenderRes<SharedDeviceState<T>>>>
    GPUCommandFuture for InitializeBufferFuture<T, TRef>
{
    type Output = ();
    type RetainedState = ();
    type RecycledState = ();
