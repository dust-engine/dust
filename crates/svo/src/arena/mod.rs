...
pub struct NodeSlot<T: Copy> {
    ...
    pub children: Handle<T>, // 32 bits
    pub data: [T; 8],
}

impl<T: Copy> NodeSlot<T> {
    pub fn child(&self, corner: Corner) -> Handle<T> {
...
pub struct Arena<T: Copy> {
    ...
    freelist_heads: [Handle<T>; 8],
    newspace_top: Handle<T>, // new space to be allocated
...
impl<T: Copy> Arena<T> {
    ...
    pub fn alloc(&mut self, len: u32) -> Handle<T> {
        ...
        let handle = if sized_head.is_none() {
            ...
            if self.newspace_top.is_none() {
                ...
                let handle = Handle::new(block_num, 0);
                self.newspace_top = Handle::new(block_num, len);
                handle
            } else {
                ...
                if remaining_space > 8 {
                    // There's still space
                    self.newspace_top = Handle::new(block_num, item_num + len);
                } else {
                    // Need to request new space
                    if remaining_space > 0 {
                        self.freelist_push(
                            remaining_space as u8 - 1,
                            Handle::new(block_num, item_num + len),
                        );
                    }
    ...
    fn freelist_push(&mut self, n: u8, handle: Handle<T>) {
        let n = n as usize;
        self.get_slot_mut(handle).free.next = self.freelist_heads[n];
        self.freelist_heads[n] = handle;
    }
    fn get_slot(&self, handle: Handle<T>) -> &Slot<T> {
        let (block_num, item_num): (usize, usize) = handle.into();
        &self.data[block_num][item_num]
    }
    fn get_slot_mut(&mut self, handle: Handle<T>) -> &mut Slot<T> {
        let (block_num, item_num): (usize, usize) = handle.into();
    ...
    pub fn realloc(&mut self, node: Handle<T>, new_mask: u8) {
...
impl<T: Copy> Index<Handle<T>> for Arena<T> {
...
impl<T: Copy> IndexMut<Handle<T>> for Arena<T> {
...
mod tests {
    ...
    fn test_free() {
        let mut arena: Arena<u32> = Arena::new();
        let handles: Vec<Handle<u32>> = (0..8).map(|_| arena.alloc(4)).collect();
        for handle in handles.iter().rev() {
            arena.free(*handle);
        }
        assert_eq!(arena.alloc(1), Handle::new(0, 8 * 4));
    ...
    fn test_invalid_overlapping_handle() {
        ...
        let handle = arena.alloc(8);
        let (block_num, item_num): (u32, u32) = handle.into();
        let handle = Handle::new(block_num, item_num + 4);
        arena.free(handle);
    }
